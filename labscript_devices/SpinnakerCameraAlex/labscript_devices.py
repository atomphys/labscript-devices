#####################################################################
#                                                                   #
# /labscript_devices/SpinnakerCameraAlex/labscript_devices.py       #
#                                                                   #
...
# This is a copy of /labscript_devices/IMAQdxCamera/labscript_devices.py,
# where we renamed the class IMAQdxCamera by SpinnakerCameraAlex (this is the same name 
# as stated in the SpinnakerCameraAlex.register_classes.py, what is important!) 
# and used description = 'Spinnaker Camera' instead of description = 'IMAQdx Camera'.
# Additionally we added a calculate-function and therefore also edited the generate_code-function
# and added self.calculations = [] in the __init__-function.
...
class SpinnakerCameraAlex(TriggerableDevice):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.exposures = []
        self.calculations = []  # added (otherwise: "AttributeError: 'SpinnakerCameraAlex' object has no attribute 'calculations'")
    ...
    # NEW FUNCTION:
    def calculate(self, calculated_image_name, image_name1, frametype1, image_name2, frametype2, box):       
        """
        With this function one can calculate an image ("calculated image") out of two images ("first" and "second" image) that are done 
        by the same camera in the current shot considering only the rectangular region one is interested in.
        The exact function calculated image = f("first" image, "second" image) is defined in the file
        /labscript_devices/SpinnakerCameraAlex/blacs_workers.py in the transition_to_manual-function of the class SpinnakerCameraWorker.
        
        Parameters
        ----------
        calculated_image_name (string):
            States the name of the resulting calculated image (i.e the calculated image after cutting).
            Important for name under which the resulting calculated image is stored in the hdf5 file.
            This should be a unique name!
        image_name1 (string):
            States the name of the "first" image according to the expose function.
        frametype1 (string):
            States the frametype of the "first" image according to the expose function.
        image_name2 (string):
            States the name of the "second" image according to the expose function.
        frametype2 (string):
            States the frametype of the "second" image according to the expose function.
        box (array consisting of 4 integers):
            States the rectangular region of the calculated image we are interested in.
            The calculated image from the function f("first" image, "second" image) is cut according to the variable 
            box = [box_left, box_top, box_right, box_bottom], where box_left states the left pixel coordinate, 
            box_top states the top pixle coordinate, box_right states the right pixel coordinate and 
            box_bottom states the bottom pixle coordinate of the calculated image.
            If one does not want to cut the calculated image, one has to set box = [0,0,x,y]
            where x is the width of the uncut images and y is the heigth of the uncut images.
        """
        box_left = box[0]
        box_top = box[1]
        box_right = box[2]
        box_bottom = box[3]
        self.calculations.append((calculated_image_name, image_name1, frametype1, image_name2, frametype2, box_left, box_top, box_right, box_bottom))

    # # This is the "old" generate_code-function:
    ...
    # This is our new generate_code-function, which next to the exposure-function 
    # also takes the calculate-function into account:      
    def generate_code(self, hdf5_file):
        self.do_checks()
        vlenstr = h5py.special_dtype(vlen=str)
        table_dtypes1 = [
        ...
        data1 = np.array(self.exposures, dtype=table_dtypes1)
        data2 = np.array(self.calculations, dtype=table_dtypes2)
        group = self.init_device_group(hdf5_file)
        if self.exposures:
            group.create_dataset('EXPOSURES', data=data1)          
        if self.calculations:
            group.create_dataset('CALCULATIONS', data=data2)
